...
if (testAxiom || runAllTests) {
    ...
    let axiomOutputs = [[gL("A"), gL("B"), gL("C")],
        ...
        [gL("A", [1, 2])],
        [gL("A", [1, 2]), gL("B")]];
    runnerHelper("Axiom", axiomInputs, axiomOutputs, parser_1.parseAxiom, compareAxiom);
    let axiomErrors = ["{", "A((1"];
    errorHelper("axiom", axiomErrors, parser_1.parseAxiom);
}
if (testSuccessor || runAllTests) {
    ...
    let successorOutputs = [gS([gL("A")]),
        ...
        gS([gL("A")], 4),
        gS([gL("A")], 10)];
    runnerHelper("Succesor", successorInputs, successorOutputs, (s, i) => parser_1.parseSuccessor(s, successorInputParams[i]), compareSuccessor);
    let successorErrors = ["{4A"];
    errorHelper("successor", successorErrors, (s, i) => parser_1.parseSuccessor(s, successorInputParams[i]));
}
if (testPredecessor || runAllTests) {
    ...
    runnerHelper("Succesor", predInputs, predOutputs, parser_1.parsePredecessor, comparePredecessor);
    let incorrectPredInputs = ["{", "B(a,", "B{x", "B<<"];
    errorHelper("Predecessor", incorrectPredInputs, parser_1.parsePredecessor);
}
if (testProduction || runAllTests) {
    let productionInputs = ["A : AF", "A<B(x)>R:B(1)A", "A(x,y){x>y}: A(x*2, y*3)F"];
    let productionOutputs = [gPd(gP(gL("A")), gS([gL("A"), gL("F")])),
        gPd(gP(gL("B", ['x']), { left: gL('A'), right: gL('R') }), gS([gL("B", [(x) => 1]), gL("A")])),
        gPd(gP(gL("A", ['x', 'y']), undefined, (x, y) => x > y), gS([gL("A", [(x, y) => x * 2, (x, y) => y * 3]), gL("F")]))];
    runnerHelper("Production", productionInputs, productionOutputs, parser_1.parseProduction, compareProduction);
}
if (testStochastic || runAllTests) {
    let stochInputs = [["A: X", "A:Y"], ["A(x): X", "A(x): Y", "A(x): B"], ["A(r): R", "A(r): {2} R"]];
    let stochOutputs = [[gPds(gP(gL("A")), gS([gL("X")]), gS([gL("Y")]))],
        [gPds(gP(gL("A", ['x'])), gS([gL("X")]), gS([gL("Y")]), gS([gL("B")]))],
        [gPds(gP(gL("A", ['r'])), gS([gL("R")]), gS([gL("R")], 2))]
    ];
    runnerHelper("Stochastic", stochInputs, stochOutputs, parser_1.parseProductions, compareProductions);
}
